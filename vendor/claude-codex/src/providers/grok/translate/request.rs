...
pub fn translate_request_with_mode(
    ...
    Ok(GrokResponsesRequest {
        ...
        reasoning: parse_reasoning(req, &model)?,
    })
}
...
fn parse_reasoning(req: &MessagesRequest, model: &str) -> anyhow::Result<Option<GrokReasoning>> {
    let effort = req
        .extra
        .get("output_config")
    ...
    if !allowed.contains(&effort) {
        anyhow::bail!("unsupported reasoning effort {effort} for {model}");
    }
    Ok(Some(GrokReasoning {
        effort: effort.into(),
    }))
}
...
#[cfg(test)]
mod comandos_reasoning_tests {
    use super::*;

    fn request(effort: &str) -> MessagesRequest {
        serde_json::from_value(serde_json::json!({
            "model":"ignored",
            "max_tokens":64,
    ...
    #[test]
    fn grok_46_forwards_xhigh_reasoning_effort() {
    ...
    #[test]
    fn grok_45_rejects_xhigh_reasoning_effort() {
        let error = translate_request_with_mode(
            &request("xhigh"),
            "grok-4.5".into(),
            GrokToolImageMode::Omit,
        )
        .unwrap_err()
        .to_string();
        assert!(error.contains("unsupported reasoning effort"));
    }
}
