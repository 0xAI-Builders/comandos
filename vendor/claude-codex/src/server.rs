...
const MAX_ANTHROPIC_REQUEST_BYTES: usize = 64 * 1024 * 1024;
...
pub fn app_with_features(
    ...
    let router = Router::new()
        ...
        .route(
            "/v1/messages",
            post(handler_messages).layer(DefaultBodyLimit::max(MAX_ANTHROPIC_REQUEST_BYTES)),
        )
        .route(
            "/v1/messages/count_tokens",
            post(handler_count_tokens).layer(DefaultBodyLimit::max(MAX_ANTHROPIC_REQUEST_BYTES)),
        )
