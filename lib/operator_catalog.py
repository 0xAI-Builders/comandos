...
CATALOG: list[ToolSpec] = [
    ...
    T("session_status", "sessions", "Estado vivo del panel seleccionado. scope=session incluye todos sus paneles. historical=true incluye filas históricas identificadas por alive=false; no son agentes activos.", P(session="Sesión exacta", pane=PANE, scope=("string", "Ámbito", ("selected", "session")), historical=("boolean", "Incluir historial")), ("session",), api("GET", "/state"), readonly=True),
    T("get_active_tab", "sessions", "Qué pestaña está activa en la app y su pane vivo.", target=api("GET", "/active-tab"), readonly=True),
    T("session_brain", "sessions", "Configuración de skills/MCPs del panel seleccionado. Resuelve carpeta, CLI y cuenta desde su identidad viva; configuración no demuestra activación ni uso.", P(session="Sesión exacta", cwd="Carpeta absoluta, debe coincidir con el panel", pane=PANE, harness="CLI", account="Cuenta"), ("session",), api("GET", "/session-brain", query={"session":"$session", "cwd": "$cwd", "pane": "$pane", "harness": "$harness", "account":"$account"}), readonly=True),
